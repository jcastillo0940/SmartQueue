...
import { MapPin, Plus, ChevronLeft, Store, Trash2, Edit2, X, CheckCircle, AlertCircle } from 'lucide-react';
                        ...
                        <div className="bg-white rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 sticky top-24">
                            ...
                            <form onSubmit={submit} className="space-y-4">
                                <div>
                                    ...
                                    <input type="text" value={data.name} onChange={e => setData('name', e.target.value)} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none bg-gray-50/50" placeholder="Ej. Sucursal Centro" required />
                                    {errors.name && <p className="text-red-500 text-xs mt-1">{errors.name}</p>}
                                </div>
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-1">Ubicación / Dirección</label>
                                    <input type="text" value={data.location} onChange={e => setData('location', e.target.value)} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none bg-gray-50/50" placeholder="Calle 50, Plaza..." />
                                </div>
                                
                                <button disabled={processing} className={`w-full py-3 px-4 rounded-xl font-bold text-white transition-all shadow-lg ${editingBranch ? 'bg-amber-500 hover:bg-amber-600 shadow-amber-100' : 'bg-emerald-600 hover:bg-emerald-700 shadow-emerald-100'}`}>
                        ...
                        <div className="bg-white rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100">
                            <div className="space-y-4">
                                {sucursales.length > 0 ? sucursales.map((sucursal) => (
                                    <div key={sucursal.id} className={`group flex items-center justify-between p-5 rounded-2xl border transition-all ${editingBranch?.id === sucursal.id ? 'border-amber-200 bg-amber-50/30' : 'border-gray-100 hover:border-emerald-100 hover:bg-gray-50/50'}`}>
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {sucursal.is_active ? 
                                                        <span className="flex items-center text-[10px] font-bold uppercase bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-md italic">Activa</span> 
                                                        : <span className="flex items-center text-[10px] font-bold uppercase bg-gray-100 text-gray-500 px-2 py-0.5 rounded-md italic">Inactiva</span>
                                                    }
                                                </div>
...
<Link 
    ...
    className="ml-2 text-sm font-bold text-emerald-600 bg-emerald-50 px-4 py-2 rounded-xl hover:bg-emerald-600 hover:text-white transition-all"
>
    Departamentos
</Link>
